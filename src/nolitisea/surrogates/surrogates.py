"""FT / AAFT / iterative AAFT surrogate generation."""
...
def iaaft(x, maxiter=1000, atol=1e-8, rtol=1e-10):
    ...
    for i in range(maxiter):
        # Match power spectrum.  Where |t| == 0 (e.g. a zero DC component
        # from a zero-mean series) the phase is undefined; use unit phase
        # so that ``ampl * phase`` gives ``ampl`` (which is itself zero
        # for the DC bin whenever the data mean is zero).
        abs_t = np.abs(t)
        phase = np.ones_like(t)
        mask = abs_t > 0
        phase[mask] = t[mask] / abs_t[mask]
        s = np.real(np.fft.irfft(ampl * phase, n=len(x)))
