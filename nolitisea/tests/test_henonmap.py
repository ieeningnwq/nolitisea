...
class TestHenonMap(unittest.TestCase):
    ...
    def test_output_shapes_custom_n(self):
        for n in [0, 1, 5, 100, 5000]:
            X, Y = henon_map(n=n, transient=10)
            self.assertEqual(X.shape, (n,))
            self.assertEqual(Y.shape, (n,))

    def test_recurrence_relation(self):
        a, b = 1.4, 0.3
        X, Y = henon_map(a=a, b=b, transient=100, n=50)
    ...
    def test_reproducibility_same_params(self):
        X1, Y1 = henon_map(a=1.4, b=0.3, x0=0.0, y0=0.0, transient=500, n=2000)
        X2, Y2 = henon_map(a=1.4, b=0.3, x0=0.0, y0=0.0, transient=500, n=2000)
        np.testing.assert_allclose(X1, X2)
        np.testing.assert_allclose(Y1, Y2)

    def test_custom_ab_parameters(self):
        a, b = 1.2, 0.2
        X, Y = henon_map(a=a, b=b, transient=50, n=30)
        for i in range(len(X) - 1):
    ...
    def test_custom_initial_conditions(self):
        X1, Y1 = henon_map(x0=0.5, y0=-0.2, transient=0, n=1)
        a, b = 1.4, 0.3
    ...
    def test_zero_transient(self):
        a, b = 1.4, 0.3
        x0, y0 = 0.1, 0.2
        X, Y = henon_map(a=a, b=b, x0=x0, y0=y0, transient=0, n=3)
    ...
    def test_transient_is_discarded(self):
        X_short, Y_short = henon_map(transient=0, n=105)
        X_discard, Y_discard = henon_map(transient=100, n=5)
        np.testing.assert_allclose(X_short[100:105], X_discard)
        np.testing.assert_allclose(Y_short[100:105], Y_discard)

    def test_no_nan_or_inf(self):
        X, Y = henon_map(a=1.4, b=0.3, transient=1000, n=10000)
    ...
    def test_n_zero_returns_empty_arrays(self):
        X, Y = henon_map(n=0, transient=10)
    ...
    def test_alternate_parameters_chaotic_region(self):
        for a, b in [(1.4, 0.3), (1.3, 0.35), (1.45, 0.25)]:
            X, Y = henon_map(a=a, b=b, transient=500, n=1000)
