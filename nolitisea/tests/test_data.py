...
class TestLorenzSystem(unittest.TestCase):
    ...
    def test_custom_initial_conditions(self):
        ...
        t, x = lorenz_map(length=50, x0=custom_x0)
    ...
    def test_zero_discard(self):
        ...
        custom_x0 = [1.0, 1.0, 1.0]
        t, x = lorenz_map(length=10, x0=custom_x0, step=0.01, sample=0.01, discard=0)
    ...
    def test_random_seed_reproducibility(self):
        ...
        np.random.seed(42)
        t1, x1 = lorenz_map(length=100)

        np.random.seed(42)
        t2, x2 = lorenz_map(length=100)
    ...
    def test_custom_parameters(self):
        """Test if the calculation runs normally after modifying lorenz_map parameters."""
        # Test that modifying parameters does not cause crashes
        try:
            t, x = lorenz_map(length=50, sigma=15.0, beta=2.0, rho=10.0)
